...
export function arePathsEqual(a: number[], b: number[]): boolean {
  if (a === b) {
    return true;
  }
...
export function hashPath(a: number[]): string {
  return a.join('-');
}
...
export class PathSet implements Iterable<number[]> {
  _table: Map<string, number[]>;
  ...
  *entries(): Iterator<[number[], number[]]> {
  ...
  forEach(func: (number[], number[], PathSet) => void, thisArg?: any) {
    for (const entry of this) {
